    ...
    <div className="bg-palette-card md:bg-transparent w-[100vw] md:w-auto px-5 flex-grow self-center lg:self-start mt-8 md:mt-0 !-mx-[1rem] lg:ltr:ml-4 lg:rtl:mr-4 py-5 md:py-0 rounded-tl-[4rem] rounded-tr-[3rem] flex flex-col z-10">
      ...
      <div className="flex items-start flex-wrap relative">
        <div className="flex-grow mt-6">
          ...
          <div className="mt-4">
          {Object.keys(product.details!).map((key: string) => {
  const detailsValue = Array.isArray((product.details as {[k: string]: any})[key])
    ? [...(product.details as {[k: string]: any})[key]].join(" - ")
    : (product.details as {[k: string]: any})[key] === true
    ? t.true
    : (product.details as {[k: string]: any})[key] === false
    ? t.false
    : (product.details as {[k: string]: any})[key];
